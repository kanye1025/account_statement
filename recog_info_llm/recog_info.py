...
class RecogInfo:
        ...
        for row in self.obj['res2'][:4]:
            ...
            for k,v in row.items():
                if '.' in k:
                    k = k.split('.')[1]
                    field_name = head[k]
                    row_data[field_name] = self.normalize_digit_str(v)
            row = self.get_fields(row_data)
            
            row['交易对手账户']["账户类型"] = self.get_account_class(row["交易对手账户"]["户名"])
            print(self.get_label(json.dumps(row,ensure_ascii=False)))
            #print(json.dumps({"origin_row":row_data,"field_row":row},ensure_ascii=False))
            
        #for
        #self.field_align()
        del self.obj['res1']['outside_infos']
        return self.obj
    def get_fields(self,row_data):
        text = []
        #if self.obj["res1"]["agent_type"]:
        #    text.append(f"""当前账户类型：{self.obj["res1"]["agent_type"]}""")
        if "account_name" in self.obj["res1"] and self.obj["res1"]["account_name"]:
            text.append(f"""当前账户名：{self.obj["res1"]["account_name"]}""")
        if "account_num" in self.obj["res1"] and self.obj["res1"]["account_num"]:
            text.append(f"""当前账号：{self.obj["res1"]["account_num"]}""")
        text.extend([f'{k}:{v}' for k,v in row_data.items() ])
        
        text = '\n'.join(text)
        prompt = Prompts.create_prompt(Prompts.FIELD_PROMPT,{"text":text})
        #print(prompt)
        print(text)
        ...
        print(obj)
        if not obj["当前账户"]["户名"]:
            obj["当前账户"]["户名"] = self.obj["res1"]["account_name"] if "account_name" in self.obj["res1"] else ""
        if not obj["当前账户"]["开户行"]:
            obj["当前账户"]["开户行"] = self.obj["res1"]["agent_type"] if "agent_type" in self.obj["res1"] else ""
        obj["当前账户"]["账户类型"] = self.get_account_class(obj["当前账户"]["户名"])
        return obj
    ...
    def get_account_class(self,account_name):
        for k,v in self.account_class.items():
            for n in v:
    ...
    def get_consume_label(self,row):
        text = list()
        if row["交易对手账户"]["户名"]:
            text.append(f"""对方账户名:{row["交易对手账户"]["户名"]}""")
        if row["交易对手账户"]["账户类型"]:
            text.append(f"""对方账户类型:{row["交易对手账户"]["账户类型"]}""")
        if row["交易对手账户"]["账号"]:
            text.append(f"""对方账号:{row["交易对手账户"]["账号"]}""")
        if row['备注/摘要/附言/说明']:
            text.append(f"""交易说明:{','.join(row["备注/摘要/附言/说明"])}""")
        if row["收支类型"]:
            text.append(f"""收支类型:{row["收支类型"]}""")
        if row["订单类型"]:
            text.append(f"""订单类型:{row["订单类型"]}""")
        if row["交易网点"]:
            text.append(f"""交易网点:{row["交易网点"]}""")
        if row["交易渠道"]:
            text.append(f"""交易渠道:{row["交易渠道"]}""")
        if row["交易方式"]:
            text.append(f"""交易方式:{row["交易方式"]}""")
        if row["交易类型"]:
            text.append(f"""交易方式:{row["交易类型"]}""")
        text = '\n'.join(text)
    ...
    @lru_cache(None)
    def get_label(self,row_str):
        row = json.loads(row_str)
        pay_type = row.get("收支类型",None)
        if pay_type not in ("收入", "支出"):
            return ""
        if row["当前账户"]["户名"] and row["当前账户"]["户名"] == row["交易对手账户"]["户名"] :#and row["交易对手账户"]["户名"] =="借记账户":
            return ("Z07账户余额","减少") if pay_type == "收入" else ("Z07账户余额","增加")
        ...
        text = list()
        #if row["当前账户"]["开户行"]:
        #    text.append(f"""当前开户行:{row["当前账户"]["开户行"]}""")
        #if row["当前账户"]["户名"]:
        #    text.append(f"""当前账户名:{row["当前账户"]["户名"]}""")
        #if row["当前账户"]["账户类型"]:
        #    text.append(f"""当前账户类型:{row["当前账户"]["账户类型"]}""")
        if '交易对手账户' in row:
            if row["交易对手账户"].get("开户行",None):
                text.append(f"""对方开户行:{row["交易对手账户"]["开户行"]}""")
            if row["交易对手账户"].get("户名",None):
                text.append(f"""对方账户名:{row["交易对手账户"]["户名"]}""")
            if row["交易对手账户"].get("账户类型",None):
                text.append(f"""对方账户类型:{row["交易对手账户"]["账户类型"]}""")
        if row.get('备注/摘要/附言/说明'):
            text.append(f"""交易说明:{','.join(row["备注/摘要/附言/说明"])}""")
        if row["收支类型"]:
            text.append(f"""收支类型:{row["收支类型"]}""")
        if row.get("订单类型",None):
            text.append(f"""订单类型:{row["订单类型"]}""")
        if row.get("交易网点",None):
            text.append(f"""交易网点:{row["交易网点"]}""")
        if row.get("交易渠道",None):
            text.append(f"""交易渠道:{row["交易渠道"]}""")
        text = '\n'.join(text)
        text = text.replace("货款","货物款")
        prompt = Prompts.create_prompt(Prompts.account_label_prompt,{"text":text,"des":des,"key":str([i for i in account_label_dict.keys()])})
        print(text)
        ...
        if obj["标签类型"] not in account_label_dict:
            return ""
        if obj["标签类型"] == "P03消费支出":
            return self.get_consume_label(row),"增加"
...
if __name__ == "__main__":
    ...
    #file_path = "data/output/建行.xls.txt"
